...
// 初期ヘルプメッセージ
var HelpMessage = "powerd by QuickTagsPlus";
...
edButtons.push(
    new edButton(
        'ed_style'
        ,'飾'
        ,'style：スタイルと装飾したいテキストを入力して下さい (alt+y)'
        ,'[style]'
        ,'[/style]'
        ,''
        ,''
...
edButtons.push(
    new edButton(
        'ed_ulist'
        ,'一覧'
        ,'ul：リストで使用する項目数を半角数字で入力し、項目の内容を入力して下さい (alt+l)'
        ,'[ul]'
        ,'[/ul]'
        ,'[li]'
        ,'[/li]'
    ...
    if (button.visibility == 1){
        switch (button.id) {
            ...
            case 'ed_fontcolor':
                ...
                ColorPalette();
                break;
            case 'ed_upload_image':
                document.write('<input type="button" id="' + button.id + '" ' + accesskey + ' class="ed_button" onclick="onLoadUploadImage();" value="' + button.display + '" onmouseover="showHelpMsg(' + i + ');" onmouseout="hideHelpMsg();" />');
                break;
            case 'ed_amazon':
                var division = "amazon";
                document.write('<input type="button" id="' + button.id + '" ' + accesskey + ' class="ed_button" onclick="edShowAmazonWindow();" value="' + button.display + '" onmouseover="showHelpMsg(' + i + ');" onmouseout="hideHelpMsg();" />');
                break;
            default:
...
function getStringCommentTags(tags, s) {
    ...
    var regUpload = /\[upload\](https?:\/\/[\-\_\.\!\~\*\'\(\)a-zA-Z0-9\;\/\?\:\@\&\=\+\$,\%\#]+\.(gif|jpe?g|png|bmp))/g;
    var regURL = /\[url\=(https?:\/\/[\-\_\.\!\~\*\'\(\)a-zA-Z0-9\;\/\?\:\@\&\=\+\$,\%\#]+)\]/g;
    var regURL2 = /\[url\](https?:\/\/[\-\_\.\!\~\*\'\(\)a-zA-Z0-9\;\/\?\:\@\&\=\+\$,\%\#]+)/g;
    var regStyle = /\[style\=([a-zA-Z0-9\:\;\ \-\_\'\!\*\(\)\/\.\,\%\#]+\;)\]/g;
    var regAmazon = /\[amazon\=(https?:\/\/.*?)\](.*?)\[\/amazon\]/g;
    var regAmazon2 = /\[amazon\=(https?:\/\/.*?)\](.*?)\[\/amazon\]/;
    ...
    for (var i = 0; i < tags.length; i++) {
        ...
        // Big
        if (tags[i] == "big") {
            s = s.split('[' + tags[i] + ']').join('<span style=\"font-size: x-large;\">');
        } else if (tags[i] == "/big") {
            s = s.split('[' + tags[i] + ']').join('</span>');
        }
        // Small
        if (tags[i] == "small") {
            s = s.split('[' + tags[i] + ']').join('<span style=\"font-size: xx-small;\">');
        } else if (tags[i] == "/small") {
            s = s.split('[' + tags[i] + ']').join('</span>');
        }
        // left center right
        ...
        // Image
            ...
            s = s.replace(regImage, '<a href=\"$1\" class=\"fancybox\"><img src=\"$1\" alt=\"');   //[image=URI]TEXT[/image]
            s = s.replace(regImage2, '<a href=\"$1\" class=\"fancybox\"><img src=\"$1\" alt=\"$1\" title=\"$1');   //[image]URI[/image]
        } else if(tags[i] == "/image") {
            s = s.replace(/\[\/image\]/g, '\" class=\"pic\" \/></a>');
        }
        // Upload
        else if (tags[i] == "upload") {
            s = s.replace(regUpload, '<a href=\"$1\" class=\"fancybox\"><img src=\"$1\" alt=\"');   //[image=URI]TEXT[/image]
        } else if (tags[i] == "/upload") {
            s = s.replace(/\[\/upload\]/g, '\" class=\"pic\" \/></a>');
        }
        // URL
        else if(tags[i] == "url") {
        ...
        // Amazon
            ...
            var matches = s.match(regAmazon);
            if (matches !== null) {
                for (var k = 0; k < matches.length; k++) {
                    var itemCSV = RegExp.$2.split("|");
                    var img = itemCSV[0];
                    var info = "";
                    for (var j = 1; j < itemCSV.length; j++) {
                        info += itemCSV[j] + "<br/>";
                    }
                    var html = '<a href="'+RegExp.$1+'">' +
                               '<div class="qt_amazon">' +
                               '<div class="amazon_l"><img src="'+img+'"/></div>' +
                               '<div class="amazon_r">'+info+'</div>' +
                               '<div class="clearfix"></div>' +
                               '</div></a>';
                    s = s.replace(regAmazon2, html);
                }
            }
        }
        // Default & Others
...
function noImagePreview(s) {
    ...
    for (var i = 0; i < tags.length; i++) {
        if (tags[i] == "image") {
            s = s.replace(regImage, '<mark class="image_no_preview">[image not display.]');   //[image=URI]TEXT[/image]
            s = s.replace(regImage2, '<mark class="image_no_preview">[image not display.]');   //[image]URI[/image]
        } else if (tags[i] == "/image") {
            s = s.replace(/\[\/image\]/g, '</mark>');
        } else if (tags[i] == "upload") {
            s = s.replace(regUpload, '<mark class="image_no_preview">[image not display.]');   //[upload]URI[/upload]
        } else if (tags[i] == "/upload") {
            s = s.replace(/\[\/upload\]/g, '</mark>');
        } else if (tags[i] == 'amazon') {
            s = s.replace(regAmazon, '<mark class="image_no_preview">[image not display.]</mark>');   //[amazon=URI]TEXT[/amazon]
        }
    }
